...
import './RepairShopCard.css';
import { FaStar, FaMapMarkerAlt, FaPhone, FaClock, FaDirections, FaMotorcycle } from 'react-icons/fa';
...
const RepairShopCard = ({ shop }) => {
  ...
  return (
    <div className="repair-shop-card">
      <div className="shop-image">
        <img src={imageUrl || '/images/default-repair-shop.jpg'} alt={name} />
          ...
          <div className="specialties">
            ...
            <div className="specialty-tags">
              {specialties.slice(0, 3).map((specialty, index) => (
                <span key={index} className="specialty-tag"><FaMotorcycle /> {specialty}</span>
              ))}
              {specialties.length > 3 && (
                <span className="specialty-tag">+{specialties.length - 3} more</span>
              )}
            </div>
          </div>
        )}
